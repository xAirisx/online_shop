              ...
              <table id="example1" class="table table-bordered">
                ...
                <tbody>
                  <?php
                    ...
                    try{
                      $stmt = $conn->prepare("SELECT *, order.id AS orderid FROM order LEFT JOIN user ON user.id=order.user_id ORDER BY date DESC");
                      $stmt->execute();
                      foreach($stmt as $row){
                        $stmt = $conn->prepare("SELECT * FROM product_order LEFT JOIN product ON product.id=product_order.product_id WHERE product_order.order_id=:id");
